...
public class Fov : MonoBehaviour
{
      ...
      Vector2[] uvs;
      int i;
      float beginDegree; float endDegree;
      float beginRadian; float endRadian;
      float uvRadius = 0.5f; Vector2 uvCenter = new Vector2(0.5f, 0.5f);
      float currentIntervalDegree = 0;
      float limitDegree;
      int count; int lastCount;
      float beginCos; float beginSin;
      float endCos; float endSin;
      int beginNumber; int endNumber; int triangleNumber;
      void Start() { mesh = new Mesh(); meshFilter = (MeshFilter)GetComponent("MeshFilter"); }
      void Update()
      {
            ...
            if (lastCount != count)
            {
                  ...
                  uvs[0] = uvCenter; lastCount = count;
            }
            i = 0;
            ...
            while (i < count)
            {
                  ...
                  {
                        triangles[triangleNumber + 1] = beginNumber; triangles[triangleNumber + 2] = endNumber;
                  }
                  if (radius > 0)
                  {
